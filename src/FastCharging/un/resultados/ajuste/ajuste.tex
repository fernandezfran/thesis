...
\ref{fig:ajustes}c. En este trabajo se consideraron sistemas LFP/nanotubos de 
carbono/grafeno (LFP-CNT-G) como materiales catódicos, con una alta capacidad, 
a velocidades de carga alta y un desempeño favorable en sucesivos 
ciclados a densidades de corriente relativamente altas, comparados con 
sistemas LFP-CNT y LFP-G. Para este caso seleccionado, los autores obtuvieron coeficientes 
de difusión, a partir de la pendiente de un ajuste lineal de mediciones de 
...
ajustar los datos experimentales del primero de ellos, con un tamaño de partícula 
de $0.35 \mu$m, se obtuvo un valor de $2.85\times10^{-13}$ cm$^2$/s
para este parámetro. Para el mismo se aprecia una discrepancia de un 
...
\done{
Otro sistema en el cual las C-rates a las que se midieron los perfiles 
galvanostáticos cubren valores de SOC$_{\max}$ que van desde completamente cargado
...
materiales catódicos. Comparandolos con sistemas LFP-CNT y LFP-G, estos 
...
de Wang \textit{et al.} \cite{wang2019high}\todo{, ya presentados en la Figura 
\ref{fig:preproc} y ahora en el formato en el cual son ajustados en la Figura 
\ref{fig:ajustes}d -> quise decir que antes se mostraba el perfil completo (en la fig 1.2 y ahora en el formato en el cual son ajustados, es decir, SOCmax versus C-rate)... Reemplazado por:}.
\done{. Estos datos ya se presentaron en la Figura \ref{fig:preproc} y ahora se muestran junto al ajuste en la Figura \ref{fig:ajustes}d.}
...
difusión de 0.025 $\mu$m \todo{se obtuvo -> obtuvieron} un $D$ de $3.51\times10^{-14}$ cm$^2$/s y una
$k^0$ de $1.87\times10^{-8}$ cm/s. El ajuste se muestra en la Figura 
\ref{fig:ajustes}e.

\done{Bak \textit{et al.} \cite{bak2011} sintetizaron nanopartículas híbridas de la 
espinela LiMn$_2$O$_4$ (LMO) con óxido de grafeno reducido de unos 0.01--0.04 
